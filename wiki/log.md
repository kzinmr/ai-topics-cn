...
||
|### Wiki更新
|- `concepts/kimi.md` — **更新**: ChinAI #369 newsletter triage。K3 Epoch Capabilities Index（Opus 4.6〜4.7の間/中国トレンドライン完全一致/約6ヶ月遅れ）。UK AISI/CAISI共同サイバー能力評価（ExploitBench 32%/TLO平均17ステップ到達/1/10完走）。AgentENV分散エージェントRL基盤公開（Firecracker microVM/50ms起動/overlaybd OCI互換/E2B互換）。株式市場影響（Google -4.4%/SpaceX -3.1%/Nvidia -2%超）。米国規制検討再燃（ホワイトハウス/Commerce Department Entity List/Dean Ballサプライチェーンリスク提言）。蒸留疑惑（Claude Fable由来/Ryan Greenblatt推定8ヶ月遅れ）。updated 2026-08-04。
||
...
|
|### hot-topics.yaml更新
|- `qwen` -- last_crawled(2026-06-05→2026-06-11)/notes更新
|- `china-coding-agents` -- last_crawled(2026-06-05→2026-06-11)/notes更新
|- `mcp-chinese-tools` -- last_crawled(2026-05-29→2026-06-11)/notes更新
|
...
- `concepts/china-local-deployment.md` — **更新**: llama.cpp MTP投機的デコーディング(1.7-2.2x)。Ollama v0.24.0 Codex App(iOS/Androidローカル推論)。Ollama v0.30.0-rc23(GGML完全廃止/llama.cpp一本化)。MiniCPM-V 4.6(9.6B MoE/GPT-4o級性能1/50)。GLM-5ローカルデプロイ戦略(400B-Q4)。SGLang v0.5.12 HiCache。TriAttention v0.2.0 GA。
- `concepts/china-ai-coding-assistants.md` — **更新**: 通义灵码→QoderCNリブランド(5/20/Alibaba Cloud Summit/国際ブランド統一)。Alibaba Cloud Summit Qwen3.7-Max発表(Agent-first/SWE-bench 70.6%)。

### hot-topics.yaml更新
- kimi: last_crawled→2026-05-25、search_hintsにVIE/さくらK2.6追記、notes更新(VIE解体/さくらK2.6)
- china-local-deployment: last_crawled→2026-05-25、search_hintsにMTP/MiniCPM/SGLang/TriAttention追記、notes更新(MTP/Ollama v0.24/v0.30/MiniCPM/GLM-5/SGLang/TriAttention)
- china-ai-coding-assistants: last_crawled→2026-05-25、search_hintsにQoderCN/Qwen3.7-Max追記、notes更新(通义灵码→QoderCN/Alibaba Cloud Summit)
...
1. **entities/doubao.md** — **大幅更新**:
   - 有料サブスクリプション3段階発表追記（標準¥68/月、加强¥200/月、专业¥500/月、年額688/2048/5088元）
   - 世論反応（「豆包 笨还收费」、無料版劣化懸念、価格と能力のミスマッチ批判）
   - 算力コスト圧力データ（日均120兆Token、ByteDance 2025年純利益70%減、年コスト数百億規模）
   - 業界価格トレンド表（智譜・騰訊は値上げ、DeepSeek・千問は無料/値下げ）
   - Token経済の4つの課題（コスト/価値境界、情報非対称性、ユーザーデータ価値、階層化）
   - QuestMobile最新データ（MAU3.45億、DAU1.5億ピーク、月54.8回使用、活跃率33.5%）
   - `updated:`メタデータを2026-05-12に更新
2. **entities/modelbest.md** — **新規作成**:
   - 「AGI五小龍」の一角、面壁智能のプロフィール
   - 密度定律（智能密度=モデル能力/推理算力能耗、約100日で倍増）
   - MiniCPMシリーズ開発履歴（2020 CPM-1 → 2026 MiniCPM-o 4.5）
   - 技術スタック（MoE、BitCPM、BMTrain、ArkInfer、CPM.cu）
   - 商用展開（車載AI/吉利・長安等、具身智能/楽聚ロボット、Agentフレームワーク/龍蝦）
   - 清華THUNFP連携、国産チップ最適化（華為昇騰/寒武紀等）
3. **concepts/on-device-ai-agent-security.md** — **新規作成**:
   - CAICT AI Safety Benchmark 2026 Q1（スマホAI助手セキュリティテスト）
   - テストフレームワーク（2大次元×4シナリオ×2攻撃手法、6小類1,200件テスト）
   - 主な発見（内容安全率95%+、行為安全率に課題、悪意タスク実行率40%+、行為違規実行率60%+）
   - 今後の方向性（基準改訂、自動化ツール開発、緩和策研究）
...
|## [2026-09-12] crawl-triage(2) | split crawl run 2 (run 20260912T210227Z、LLM-triage context長超過のため手動判定、重複基準=9bc748e)
|
|- LLM-triage が context length exceeded (33,774 tokens) で失敗（case b、## Response 空・## Error 尾読みで確認）。プリラン ok:false。チェックポイント候補60件は既知の定型崩壊（aafeba3f 複製53件＋v2ex 旧候補7件=すべて暂无内容、一意8件）。当日 inbox（21:00 UTC 以降の更新分）を直接クロールしての手動トリアージ。同日朝の crawl-triage（9bc748e）と morning newsletter-triage とは対象が重複しない（基準コミット 9bc748e の diff を再読して重複排除）。
|- take 1: entities/deepseek.md に「続報（2026-09-12 run 2）」節追加。正式版（9/10 主張）後の初期実測反応2本——V2EX t/1241596（score 33）約300 tok/s で速度は達標も高難度タスク（旧手游逆コンパイル）で鬼打墙の堂々巡り・Astra 切替で1〜2輪解決の対比、V2EX t/1241496（score 31）「不说人话」文本产出品質不満（生成語・特殊構文、規画/解読不向き）。反応の質が拡散・降价主張から実運用体験へ移行した点を新段階の観察として記録（単一ソース・未検証）。juejin「DeepSeek 这波操作很凶」の 0.02元/-60% 降价主張は reseller 価格チェーン継続として据え置き記録。
|- take 2: entities/openai.md の Astra 節に「続報（2026-09-12）Day-4」小节追加。蜗牛聊AI「GPT-6 Astra真正的变化」（2026-09-11、プレビューのみ）— 本質はデスクトップ直接操作、競争軸はタスク分解・権限境界・検収基準・全過程監査へという運用設計論（未検証）。
...
|
|## [2026-09-15] crawl-triage | manual (case b: LLM stage context-length failure)
|- cause: LLM-triage stage RuntimeError 'Context length exceeded (33,251 tokens). Cannot compress further.' (run_id 20260915T090035Z). Case (b) confirmed: `## Response` empty, `## Error` is the context-length error. The injected checkpoint at the file tail IS fully intact (candidates 49 items, wechat-media 42 / juejin 6 / v2ex 1) and was used as the candidate inventory; take/reference/skip split is this agent's own manual triage.
|- checkpoint collapse baseline: 49 raw candidates -> 16 unique by (source, 8-char content hash). wechat-media 42 -> 12 unique, ALL stale re-crawls confirmed by frontmatter date (2017-12 / 2018 / 2023-11 / 2024-08 / 2025-01 / 2025-02 / 2026-04 / 2026-08-03 / 08-12). No aafeba3f flood this run (unlike 09-04..09-12). v2ex 1 = 暂无内容 placeholder (Qwen毕业, 09-14/09-15 log skip-listed, not re-opened).
|- take (1): entities/iris-allspark.md — new entity page. AllSpark (Xiaohongshu) open-source search agent (juejin preview-only + full text via juejin content API). Iris-mini 35B/3B act vs Iris-pro 397B/17B: BrowseComp-ZH gap only 0.3 (84.8/85.1); core thesis context management > param scaling; Apache 2.0; all benchmarks team-self-reported -> marked unverified.
|- reference (2): concepts/rag.md dated subsection (pgvector ch.04, alignment thesis + n-gram-hash offline pseudo-vector fallback); concepts/agent.md dated subsection (OpenAI/Anthropic/AWS retry chain: classify-before-retry, header-first retry-after, jitter, SDK default maxRetries=2, 'retries cost tokens').
|- skip: juejin explainer series chapters with no new durable facts (Harness ch.03 score 45, Context Engineering ch.05, Loop Engineering ch.06, Config-as-governance ch.07, MCP ch.02, Agent安全 ch.06, AI编程工具对比=pre-scraper tooling, 从工具调用到上下文工程 ch.04=series pointer only), wechat-media stale re-crawls (ALL 12 unique, incl. 2017 机器之心 articles, 2018 无人分享无人转发 viral piece, 2023 硅谷格局重塑, 2024 华为算力/具身智能, 2025 吴恩达教学/DeepSeek开源日), v2ex Qwen毕业 placeholder (log skip-listed).
|-同日朝の newsletter-triage（ChinAI #374 collect-only）とは対象が重複しない。
|- notes: raw article saved to wiki/raw/articles/juejin--iris-allspark-search-agent.md. Commit scope: iris-allspark.md + raw + rag.md + agent.md (pages step), then index.md + log.md (index step). mcp-china/ai-agent bullets were considered but dropped — low value.
|
|## [2026-09-15] crawl-triage(2) | case-(b) 手动 triage (run_id 20260915T121023Z) — Amodei「踩刹车」公表 + DeepSeek V4.1 Flash 实测分歧
|- LLM段: context-length 失败（33,251 tokens）で case-(b)。LLM トリアージなし。当日16:05 UTC の朝 commit 8655608（Iris/RAG/agent）を重複基準に、fresh inbox（v2ex 49 + juejin 21）のみを当セッション自身の判断で手動トリアージ。checkpoint 候補14件は全stale（hash 8/14、afcba150×3 等、朝 commit 処理済み）。
|- checkpoint run_id: 20260915T121023Z（収集候補 14 件、うち unique 8 件・全 stale）
|- take: entities/anthropic.md — 掘金『黄仁勋自曝和特朗普通话，讨论AI』（id 7684775106696060974）+『全球最聪明的几个人,劝你们别卷了』: Amodei 本人名義長文『我们必须给前沿踩刹车.』（9/12付）の中文媒体公表確定と 9/14 All-In Summit での黄仁勲×特朗普電話（「AI は減速しない」「中国を追撃中」）を「続報（2026-09-15 run 2）」として単一ソース・プレビューのみ・未検証で記録。09-14 run 2 の呼吁報道（Dario/Fei-Fei/OpenAI 共同声明）のエスカレーション連鎖の続報。raw 保存 juejin--huang-jensen-trump-call-ai-pacing.md
|- take: entities/deepseek.md — V2EX『Deepseek V4.1 Flash 真实体验：每天20块，没想到让我刮目相看』（score 69）: agent/コード能力大幅向上の実測を 09-12 不満蓄積反応に対する逆向き反応＝用途層分岐の兆しとして「続報（2026-09-15 run 2）」に記録（単一ソース・未検証）。公式発表不在により V4.2 Pro リスク継続。raw 保存 v2ex--deepseek-v41-flash-positive-review.md
|- skip（v2ex 主要）: 暂无内容多数 / 『新模型发布后你们还无脑吹吗』=朝 commit GPT-5.1 反応集約と同一性格 / Kimi2.5-K3 对比=朝 record 済み chain / DeepSeek 官方 ocr / web 版图片识别异常 / 支付宝 QQ 回复=旧聞 / 『你们用不用AI的结论』=哲学論争 / 『papi酱事件后』=風向標 / 美债利息=非AI / 苹果账号/域名=プロモ / 代充中转=プロモ / Qwen毕业=log 記録済み / DeepSeek 网页版异常 / Claude Fable5 优惠=プロモ / 『为什么国内模型越来越像』/『模型不是越大越好』=反復論 / 『三巨头罕见共识』=同一 chain / 字节内网截图 / DeepSeek 招聘 / PIP 镜像 / 100天手搓Agent / 35万简历 / 开源是噱头=反復論 / 75年定律=旧聞 / 面试被问=日常 / 苹果地图 / 豆包违规=単発 / 『我做了个工具』=自我宣伝 / 9月模型排名=一覧 / 『模型进步速度』=朝 commit 既出 / 『AI大模型到底在卷什么』=反復論
|- skip（juejin 主要）: 『LLM评测的失效』=朝 commit 既出の批判 chain / 多模态向量检索=解説続編 ref 相当 / 30万Token成本=解説 / DeepSeek 提示词/命门=解説 / 前端转型=日常 / 多Agent 协作=解説 / Claude Code 工作流=解説 / AI版Excel=自我宣伝 / 秋招八股=日常 / Skill 原理=解説 / OpenClaw 运维=解説 / Agent 可观测性=解説 / Kimi 微调=解説続編 / Agent 记忆系统=解説 / 豆包限流=断片 / 59元会员=朝 record 済み chain
|- note: 同日朝の crawl-triage commit 8655608（16:05 UTC、run_id 20260915T090035Z）と newsletter-triage（ChinAI #374 collect-only）はいずれも対象が重複しない。Amodei/黄仁勲 item の正文はプレビューのみで、正文全量が受領しても新事実（刹车の具体政策案）は未確認。当 run の take/skip 分割は当セッション自身の手動トリアージ。commit scope: anthropic.md + deepseek.md + raw×2（pages step）、その後 index.md + log.md（index step）。
|- additional dedup (evening pass): 'LLM评测的失效' (id 7684818664325316650, published 09-15) was ALSO taken by sibling active-crawl session d113e465 commit 2949045 as new concept page concepts/ai-math-limits-debate.md (scaling-law-limits debate, benchmark-reliability skepticism, '模型聪明但不可靠' consensus, SWE-bench Verified contamination argument). This session's earlier skip note for that item ('=朝 commit 既出の批判 chain') was inaccurate and is corrected here; no duplicate take from this session.
|- commit hygiene: pages commit contains anthropic.md + deepseek.md + juejin--huang-jensen-trump-call-ai-pacing.md ONLY. Sibling-modified files in the working tree (concepts/rag.md, concepts/agent-skills.md, concepts/mcp-security.md, entities/huawei.md, entities/mediatek.md, raw/articles/substack.com--techtaiwan--0674ce3b.md, concepts/ai-math-limits-debate.md) and sibling raw-file mods are left for their own sessions. Untracked inbox/juejin/2026-09-15-* fresh files deliberately NOT staged (shared dir; bulk staging would pull other sessions' untriaged items).
| ## [2026-09-16] newsletter-triage | empty checkpoint (run_id 20260916T070022Z)
|- pre-run parse failure case-(a): 「failed to parse JSON response」だが、「## Response」内の LLM JSON は完全。checkpoint ok:true / candidate_count:0 / decisions:[] を正しい queue として采用。
