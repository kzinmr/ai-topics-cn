...
36kr（36氪）は**「估值超OpenAI，Anthropic逼近万亿美金」**と報道。Anthropicの第二次市場評価は**1兆ドル（約150兆円）**に達し、OpenAIを初めて超えた。

- 第二次市場評価: **$1T**（OpenAI超え）
- 36krは「Anthropicの估值がOpenAIを初めて超えた」
- 中国開発者コミュニティは「AnthropicのペースがOpenAIを追い抜いた」と評価

> **出典**: 36kr — [https://36kr.com/p/3778903190639617](https://36kr.com/p/3778903190639617) [T1]

### Claude Mythos — 未発表モデルの「非法访问」事件

36kr（财联社AI daily）は**「Anthropic顶级模型Mythos遭非法访问，未公开即泄露」**と報道。Claude Mythos（Claudeの最上位モデル）が未発表段階で外部に**非法アクセス**され、内容がリークした可能性。

- **Mythos**: Anthropicの最上位モデル、未発表
- **事件**: 外部による非法アクセスが検知
- **影響**: Claudeのセキュリティ体制への懸念
- 中国語圏では「Mythosのセキュリティが心配」（Mythos的网络安全令人担忧）と議論

Mythosは既にOpenMythos（[[openmythos]]）として一部アーキテクチャが逆推・开源されているが、今回の事件はより深層のモデル内容が外部に漏洩した可能性を示唆。
