...
## CANN 8.0と華為のCUDA対抗戦略（2026年6月 ChinAI #361分析）

### CUDAの「3つの頭」を持つヒドラと華為の挑戦

NVIDIAの優位性は、**ハードウェア**、**ネットワーキング**、そして最も重要な**ソフトウェアエコシステム（CUDA）**という「三頭のヒドラ」に支えられている。CUDAは2007年にリリースされた並列コンピューティングプラットフォームで、NVIDIAはコンシューマーGPUで開発者基盤を育成する「鶏と卵」問題を克服し、AlexNet（2012年）以降のディープラーニング革命で支配的地位を確立した。

### 華為の三本柱戦略

華為はCUDAエコシステムからの脱却に向け、以下の3つのアプローチを並行して進めている：

**1. CANN（Compute Architecture for Neural Networks）— 独自スタックの構築**
- CANN 8.0（2024年9月リリース）はAscend NPU向け並列プログラミング環境
- MindSporeはPyTorchに相当する高レベル深層学習フレームワーク
- **開発者コミュニティの課題**: CANNは依然として「幼児期から青年期への移行段階」（AIGCode陳秋武の表現）。Zhihuでは「2027年まで成熟しない」との声も
- 華為は百度・iFlytek・騰訊にエンジニアを派遣し、CUDAベースの訓練コードをCANN環境で再実装する「手厚いサポート」を提供（NVIDIAの初期CUDA戦略と同じアプローチ）
- DeepSeekエンジニアはAscend 910CでH100の推論性能の**約60%**を達成可能性を報告

**2. PyTorch統合（torch_npuアダプター）**
- torch_npuはPyTorchと華為のNPUドライバ/CANNバックエンドを接続するアダプター
- 2023年10月、華為はPyTorch Foundationのプレミアメンバーに加入（Meta、NVIDIA、AMD、Googleと並ぶ）
- **課題**: PrivateUse1メカニズムを使用するためPyTorch本流コードベースの外に存在。PyTorchの更新に自動追従できず、互換性問題が頻発
- 2024年上海ミートアップで華為エンジニアが「プラグイン版はネイティブではない」と課題を認める

**3. ONNX（Open Neural Network Exchange）戦略**
- ONNXは2017年にMetaとMicrosoftが開発したフレームワーク間モデル互換性標準
- 華為はAscend ONNX RuntimeをGitHubで公開・メンテナンス
- **戦略的意義**: NVIDIAハードウェアで訓練したモデルをONNX形式でエクスポートし、Ascendチップで推論実行可能に。訓練はCUDAバインドのまま、推論段階で華為エコシステムに参入する「デカップリング」アプローチ

### AIGCodeケーススタディ — 「芯模协同」の実証

DeepSeek V4の昇騰デプロイメントは、単なるハードウェア置き換えではなく**「芯模协同」（チップとモデルの共進化）**の新段階を示した：

- **AIGCode陳秋武**（創業者）：2024年初期、Ascend 910Bでの7B級事前訓練時「生態系はほとんど砂漠状態」だったが、8ヶ月後にはCANNカバレッジが**80〜90%**に到達
- **MFU 65%達成**: Ascend超ノード（統一メモリアドレス+高速インターコネクト）により、千卡規模クラスターで高い負荷率を維持。業界平均の約2倍
- アダプタ期間：当初は華為へのチケット提出から解決まで**3〜4ヶ月** → 現在は自行解決可能に
- 15分でシステム一式生成するAIコーディングエージェント製品をAscend上でローンチ

### 金融機関での生産利用とvLLM-Ascend貢献

- 某頭部股份制銀行がAIを**コアリスク管理フロー**に投入
  - 初Token応答: **500ms**
  - 日次処理: **260億トークン**
  - 可用性: **99.999%**（年間故障時間1分以内）
  - 大小モデル混合アーキテクチャで資金流通・リスク判断に介入
- **vLLM-Ascendへの貢献**: 同銀行が複数コミュニティに累計**100件以上**の特性を貢献。うちvLLM-Ascend単体で**34件**の最適化
- CANNオープンソース化により、金融業界のような「非典型的」開発者も底层能力建设に参加可能に

### 開発者エコシステム規模（2026年6月時点）

| 指標 | 数値 |
|------|------|
| 鯤鵬開発者 | 415万人超 |
| 昇騰開発者 | 410万人超 |
| openEulerインストール数 | 1,600万套超 |
| 主要大モデルのDay 0対応 | 70種類超 |
| CANNソースリポジトリ解耦合 | 65件 |

> **出典**: ChinAI #361 — Mary Clare McMahon「Can Huawei Take On Nvidia's CUDA?」(2026-06-01) [T1]
> **出典**: 量子位(QbitAI)「DeepSeek V4と昇騰の芯模协同」— 鯤鵬昇騰開発者大会2026レポート (2026-06-01) [T3]
