...
## 2026年6月11日〜8月12日 — 理解深化期：MCPエコシステムの成熟とセキュリティ注目

### 概要

2026年6月11日から8月12日までの62日間、中国MCPエコシステムは**「インフラ発表期」から「理解深化期」に移行**。5月の集中的なリリース（钉钉MCP广场6000+、腾讯云MCP Gateway、新华财经MCP矩阵等）の後の**消化・定着フェーズ**。主要プラットフォームは目立った新規MCP Server追加を見せず、教育コンテンツ・セキュリティ議論・MCP仕様の最終仕様化が中心になった。

### 1. MCPプロトコル：Stateless最終仕様（7/28予定→未リリース）

| 項目 | 詳細 |
|------|------|
| 発表 | 2026-05-21（RC発表）、最終仕様は7/28にリリース予定 |
| 開発 | Google Cloud Run/Kubernetesチームと共同開発 |
| 主な変更 | セッションID廃止、SSE→Streamable HTTP、OAuth 2.1強制、必須ルーティングヘッダー |
| 状態 | 8月12日時点で**リリース確認なし**（V2EX 7/26で「7/28はRC版として扱う」の発言あり） |

### 2. 中国MCPプラットフォーム最新状況（8大広場）

| プラットフォーム | 運営元 | サーバー数 | 特徴 |
|----------------|--------|-----------|------|
| **百度智能云 MCP World** | 百度 | 56,757+ | 国内最多収録 |
| **MCP 星球** | 第三方 | 54,555+ | 集約型最多 |
| **讯飞星辰 MCP 广场** | 科大讯飛 | 16,318+ | 音声AI特化 |
| **AIbase MCP 资源站** | 第三方 | 13,784+ | GitHubミラー |
| **魔搭社区 MCP 广场** | 阿里云(ModelScope) | 9,227+ | 中国最大コミュニティ |
| **心流开放平台 MCP 市场** | 第三方 | 3,852+ | データ透明 |
| **腾讯云 MCP 广场** | 腾讯云 | 1,089+ | 微信生態統合 |
| **阿里云百煉 MCP 市場** | 阿里云 | 184+ | 精選型 |

> 出典: 2026年5月時点 [T2]

### 3. 新規MCPサーバー・ツール（6月〜8月）

| ツール | 開発元 | 特徴 | 日付 | 出典 |
|--------|--------|------|------|------|
| **iOS SandboxServer** | xinghelee | iOS用APPレベルMCPデバッグツール | 8/4 | V2EX |
| **DataZen v0.0.8** | flyxl | Tauri v2 + Rust DBクライアント、MCPワークフロー搭載 | 8/7 | V2EX |
| **Linkly AI Note** | blueeon | MCP対応ローカルMarkdownノート、MCP隧道機能 | 8/4 | V2EX |
| **codebase-memory-mcp** | 冬奇Lab | LightRAGベース3路リコールコード知識庫MCP | 8/7 | 掘金 |
| **Grok Build** | xAI | オープンソースcoding agent CLI、ネイティブMCP対応 | 7/26 | 掘金 |
| **xAgent** | コミュニティ | タスク完了志向のサーバー型マルチユーザーAI Agent平台 | 7/15 | V2EX |

### 4. MCPセキュリティ動向

#### 既知CVE一覧（2026年4月〜5月発見）

| CVE | 対象 | CVSS | 状態 |
|-----|------|------|------|
| CVE-2026-30615 | MCP STDIO RCE脆弱性（全SDK/クライアント） | Critical | Anthropic「設計通り」と修正拒否 |
| CVE-2025-66335 | Apache Doris MCP SQLインジェクション | - | パッチ済み(v0.6.1) |
| CVE-2026-5058 | コミュニティMCPサーバー コマンドインジェクション | 9.8 | 銀行SEC 8-K開示 |
...
| CVE-2026-6599 | LangFlow MCP | 9.8 | パッチなし |

#### 6月〜8月のセキュリティ新動向
- **MCP固有の新規CVE**: 6月〜8月に**新報告なし**（5月の波以降）
- **LiteLLM脆弱性警告**（8/4、36kr）: 「建議立即检查并移除LiteLLM」と報道
- **ClawHubサプライチェーン攻撃**: 341個の悪意あるスキル（全体12%）がAPIキー・認証情報を窃取
- **MCP攻撃対象領域3倍拡大**: Trend Micro調査で露出MCPサーバー1,467台（前回492台から3倍）。48%のソースコードが.envに秘密情報を平文保存

### 5. 重要記事・知見（6月〜8月）

| 記事 | 著者/出典 | 内容 | 日付 |
|------|----------|------|------|
| MCP Tool/Resource/Prompt三機能解説 | 掘金 | MCPはToolのみでなくResource/Promptの3機能を持つ。GitLab MCP開発者の反省記事 | 8/4 |
| 当MCP Server变了 | Loocor/V2EX | 能力漂移（Capability Drift）問題。Server更新時の設定持続性 | 7/26 |
| MCP企業級デプロイガイド | 冬奇Lab/掘金 | セキュリティ、認証、バージョン管理 | 7/15 |
| Agent Skills完全指南 | 掘金 | SKILL.md最適化、漸進式加載。字節跳動/滴滴面接テーマに | 8/8 |
| LLM Agent底层揭秘：JSON-RPC 2.0 | 掘金 | MCP下位レイヤーJSON-RPC 2.0の工業級解説 | 8/5 |
| 大模型工具调用とMCP | ltl/掘金 | OpenAI function callingからAnthropic MCPまで、セキュリティ境界を網羅 | 8/8 |

### 6. エコシステム統計（最新）

| 指標 | 数値 | 日付 |
|------|------|------|
| SDK月間ダウンロード | **3億回**（年初1億→3億） | 4月末 |
| 公開MCPサーバー全世界 | **9,400+**（前年比7.8倍） | 5月 |
| エンタープライズ採用率 | **78%**（本番環境） | Q1 2026 |
| 中国AI Agent市場規模 | ¥4,490億($62B)/年（前年比+107%） | 5月 |
| GitHub MCP関連リポジトリ | **420**（週間成長率4.7%） | 5/20 |
| AAIF会員組織数 | **190** | 5/18 |

### 7. 総合評価

中国MCPエコシステムは2026年6月〜8月にかけて、**インフラ発表から理解深化・教育コンテンツ充実期に移行**。5月の大型リリースラッシュ後、主要プラットフォームは静かに実装を進めている。次の注目ポイントは：
- **MCP Stateless最終仕様**: 7/28予定だが未リリース。次回の主要マイルストーン
- **DeepSeek API値上げ**（8/6）: MCPエコシステムに間接的な影響（ローカルデプロイ加速）
- **セキュリティ**: MCP攻撃対象の3倍拡大とClawHubサプライチェーン攻撃は深刻化

**出典**: daily_digest 2026-07-15/26, 2026-08-08/09/10/11 (V2EX/Juejin/36kr/Trend Micro) [T1/T2]
