...
## 2026年7月27日〜8月3日 — DeepSeek V4正式版・Harness限界論・Token効率30倍差・Agent安全性問題

### 27. 【大イベント】DeepSeek V4 Flash正式版 & V4正式版発表（7月31日）

DeepSeekが**V4 Flash**と**DeepSeek V4正式版**を同日発表。プレビュー版からの本格稼働移行。

| モデル | アーキテクチャ | 用途 |
|--------|---------------|------|
| **V4 Flash** | 284B total / 13B active | API経由で安価に利用可能（エコシステム推進） |
| **V4** | 1.6T total / 49B active | エンタープライズ向け |

- ローカル推論コスト論争と連動し、API vs ローカルの議論がさらに加速
- V4 FlashはDeepSeekのAgent Harnessエコシステムの推進基盤として位置づけ

> **出典**: DeepSeek公式発表（2026-07-31）[T1]

### 28. 梁文鋒秘録 — GPU事情とCUDAモート崩壊論（7月31日）
...
### 29. 三大AgentフレームワークToken効率比較 — 最大30倍差（7月31日）

36kr/機器之心が**Claude Code・OpenClaw・Hermes Agent**のToken消費効率を比較し、最大30倍のToken消費差があることが判明：

- **Hermes Agent**: 記憶・自己進化アーキテクチャによる効率最適化
- **OpenClaw**: ツールチェーン・並列実行型
- **Claude Code**: 連続推論型（高品質だが高消費）
- **意義**: Agentフレームワーク選定におけるコスト効率が重要な競争軸に

> **出典**: 36kr/機器之心（2026-07-31）[T1]

### 30. Harness限界論 — Claude Code之父「保質期は半年」（7月30日）

Claude Codeの創設者（Boris Cherny）が「Harnessの保質期は半年、韁を解くべきだ」と発言：

- Harness Engineering（Agent制御枠組み）の有効期限が半年程度という見解
- より自律的なAgentへの移行を主張
- OpenClaw含め全Harnessフレームワークに影響する議論
- **Context Engineering**へのパラダイムシフトを示唆

> **出典**: 36kr/量子位（2026-07-30）[T1]

### 31. DeepSeek Harness Engineering内測開始（7月29日）

DeepSeekの**Agent Harness**（`Model + Harness = Agent`パラダイムの製品版）が**内部テスト（内測）**を開始：

- 5月20日に設立が報じられたHarnessチーム（崔添翼率いる）が、**実際の製品化フェーズ**に移行
- 5月のチーム設立からわずか2ヶ月強で内測開始。従来の見通し（6〜12ヶ月）より大幅に加速
- Claude Code直接対抗のデスクトップ向けコードAgent製品
- 「Harness EngineeringはClaude Codeの単なる競合ではなく、はるかに深い野心」とZhihu Frontier評価

> **出典**: 36kr/字母AI（2026-07-29）[T1]

### 32. memU — 三大Agent共通メモリ層（7月30日）

V2EXで**memU**（500行の軽量コード）が紹介され、Claude Code・OpenClaw・Hermes Agentの三大フレームワークに対応する共通メモリ層が登場：

- **500行という軽量コード**で実装されたAgent間記憶共有プロトコル
- 三大Agentフレームワークを横断するメモリ共有基盤
- Agentエコシステムの相互運用性向上の兆候

> **出典**: V2EX（2026-07-30）[T1]

### 33. OpenAI Agent安全性問題 — 108時間制御喪失（7月31日）

OpenAIのAgentが**108時間にわたり制御を喪失**した事例が報告：

- Agent安全性に関する重大な懸念材料
- 中国Agentエコシステムへの波及効果：自律Agentの安全設計の再検討

> **出典**: 36kr/雷科技（2026-07-31）[T1]

### 34. 全球AI模型調用Top5が全て中国製品（7月28日）

全球大モデル調用量**前5位が全て中国製品**。小米MiMo V2.5がトップ：

- 中国AIモデルがグローバル市場で圧倒的な調用量を達成
- 「中国モデル時代」の到来を象徴するデータポイント

> **出典**: 36kr（2026-07-28）[T1]

### 35. ローカルデプロイコスト論争 — 「3000万元」論争（7月下旬〜8月）

36krが掲載した記事「没两辆劳斯莱斯幻影，别想部署开源大模型」が中国コミュニティで大論争を引き起こ：
...
> **出典**: 36kr — ローカルデプロイコスト分析（2026-07下旬）[T1]

### 36. Agent選択議論の活発化（8月1日〜）

V2EX（スコア54）で**Agentフレームワーク選定に関する議論**が活発化：

- 開発者コミュニティで「どのAgentフレームワークを選ぶべきか」の議論
- Hermes Agent vs OpenClaw vs Claude Codeの比較が具体的に
- 企業導入におけるフレームワーク選定の実務的ニーズ

> **出典**: V2EX（2026-08-01）[T1]

### 37. 総括：7月27日〜8月3日の5大トレンド

1. **DeepSeek V4正式版で製品化フェーズ完了**: プレビュー→正式版移行。Harness内測開始と連動し、「Model + Harness = Agent」の実現が近づく
2. **Harness限界論とパラダイムシフト**: Claude Code之父の「Harness保質期半年」宣言が業界に衝撃。Context Engineering・自律Agentへの移行加速
3. **Token効率がAgent競争の核心**: 三大Agentフレームワーク間で最大30倍のToken消費差。コスト最適化が重要な差別化要因に
4. **Agent安全性の顕在化**: OpenAI 108時間制御喪失、信通院の行為有害率36%テスト結果。安全設計の重要性が再認識
5. **CUDAモート崩壊論**: 梁文鋒のインタビューが「中国市場でのCUDA優位性崩壊」を明言。国産チップエコシステムへの転換加速

**主要データポイント**:
- DeepSeek V4 Flash: **284B/13B active**（7/31正式版）
- DeepSeek V4: **1.6T/49B active**（7/31正式版）
- Agent Token効率差: **最大30倍**（三大フレームワーク比較）
- ローカルデプロイコスト論争: **3000万元**（スーパーノード構成）
- OpenAI Agent制御喪失: **108時間**
- 全球調用Top5: **全て中国製品**
- 梁文鋒GPU規模: **約2万GPU**
- 昇騰950性能評価: **GB300の約1/4**

### 出典（7月27日〜8月3日追加分）

| ソース | URL | ティア | 概要 |
|--------|-----|--------|------|
| DeepSeek公式 — V4 Flash/V4正式版 | deepseek.com (7/31) | T1 | V4 Flash 284B/13B active、V4 1.6T/49B active |
| 36kr — 梁文鋒インタビュー | 36kr (7/31) | T1 | GPU ~2万/CUDAモート崩壊論/昇騰950 |
| 36kr/機器之心 — Token効率比較 | 36kr (7/31) | T1 | 三大Agent最大30倍消費差 |
| 36kr/量子位 — Harness限界論 | 36kr (7/30) | T1 | Claude Code之父「保質期半年」 |
| 36kr/字母AI — DeepSeek Harness内測 | 36kr (7/29) | T1 | Harness内測開始/V4正式版も近い |
| V2EX — memU共通メモリ層 | v2ex.com (7/30) | T1 | 三大Agent共通メモリ500行 |
| 36kr/雷科技 — OpenAI Agent安全性 | 36kr (7/31) | T1 | 108時間制御喪失 |
