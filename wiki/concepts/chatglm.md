...
- **GLM-5.3-Flash が「New」タグで正式掲載・原生多模态モデルと判明（公式ドキュメント、9/11直接取得）⭐**: docs.bigmodel.cn のモデルラインナップで **GLM-5.3-Flash** が「New」タグ付き（GLM-5.3 は「Hot」）で掲載され、専用ページ `/cn/guide/models/vlm/glm-5.3-flash`（9/9 更新stamp確認）が公開されている。8/27 時点では API 提供段階のみの確認だったが、現在は独立ドキュメントを持つ正式モデルとして確立。公式ページの主要仕様：
  - **GLM-5 系列初の原生多模态モデル**: 入力モダリティは動画・画像・テキスト・ファイル、出力はテキスト。コンテキスト **1M**、最大出力 **128K**。テキストパラメータは GLM-5.3 と互換
  - **総パラメータ 320B / 激活 18B**、稀疏注意力＋線形注意力の混合アーキテクチャを採用した**初のオープンソース・フロンティアモデル**と公式に明記。GLM-5.3 比で注意力計算量 **1/3.01**、KVキャッシュ **1/4.44**
  - **視覚 Coding**: 視覚能力を Coding ループに原生統合。UI・レンダリング結果・操作フィードバックを能動的に観察して改善（「代码循环中的视觉反馈」）。PPTX/PDF/DOCX/XLSX 等の成果物交付も標榜
  - **GLM Coding Plan 全量上線・額度3倍**: 「已全量上线 GLM Coding Plan、额度翻 3 倍」を公式ページで確認。「跑在国产芯片上」（国産チップでの実行）を独立セクションとして掲げる
- **コミュニティ反応（V2EX、T1）**: 9/7 スレッド「Zcode 自動化怎么提前自动安排定时任务榨干 glm-5.3-flash 免费时间？」（score低）— GLM-5.3-Flash の無料時間帯を Zcode の定時タスクで自動的に使い切る方法の質問。無料時間帯枠の存在と、free tier を自動化で「榨干（搾り取る）」実務がコミュニティに定着しつつあるシグナル。低スコアのため傍証扱い。
- **続報（静穏確認）**: 9/10 の V2EX 個人体験帖（闲鱼での token 転売経験談）内に「5月頃は智谱の資源が足りず API が終日レート制限だった」との過去回想提及あり — 単一ソースの体験談であり事実認定はしない（未検証）。
- **位置づけ**: GLM-5.3 リリース（8月）後の静穏期。固有のメジャーリリースは確認されず、動向は GLM-5.3-Flash のドキュメント整備（原生多模态・320B/18B 混合アーキテクチャの公式明記）と Coding Plan への統合深化に限定。オープンウェイト（HuggingFace/ModelScope）公開の有無は今回も公式ドキュメントからは確認できず未解決。

- **出典**: 智谱BigModel公式ドキュメント docs.bigmodel.cn/cn/guide/models/vlm/glm-5.3-flash（2026-09-11 直接取得）[T1]、V2EX t/1240212（2026-09-07収集）[T1]、daily-digest 2026-09-06〜09-10 [T1]
