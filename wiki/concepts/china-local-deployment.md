...
## 2026年7月24日〜8月2日更新 — Kimi K3 OSS化と国産大模型市場の構造変化

### Kimi K3 OSS化 — 2.8兆パラメータMoEのオープンソース化（7/26-28）

Moonshot AI（月之暗面）が**Kimi K3**をオープンソース化。中国AIコミュニティで爆発的議論を引き起こした。

**アーキテクチャ詳細**:
- **総パラメータ**: 2.8兆（2.8T）
- **アーキテクチャ**: KDA + AttnRes
- **MoE構造**: Stable LatentMoE — 896エキスパート中16をアクティブに選択
- **量子化**: MXFP4ウエイト + MXFP8活性化
- **コンテキスト長**: 100万トークン
- **推論API料金**: $0.30/MTok（キャッシュヒット時）
- **推奨構成**: 64 accelerator以上のスーパーノード

**コミュニティ貢献**: vLLMコミュニティにKDAプレフィックスキャッシュを寄付。SGLang/vLLM両方でDay-0対応。

> **出典**: Kimi公式リリース（2026-07-26〜28）[T1]; 量子位・机器之心報道 [T1]

### DeepSeek V4 Flash正式版・V4正式版発表（7/31）

DeepSeekが**V4 Flash**の正式版（API公測開始）と**DeepSeek V4正式版**を同日発表。

- V4 Flash: 284B total/13B active — API経由で安価に利用可能
- V4: 1.6T total/49B active — エンタープライズ向け
- ローカル推論コスト論争と連動し、API vs ローカルの議論がさらに加速

> **出典**: DeepSeek公式発表（2026-07-31）[T1]

### DeepSeek創設者梁文鋒秘録 — GPU事情とCUDAモート崩壊論（7/31）

36kr等がDeepSeek創設者梁文鋒のインタビュー詳細を報道：
...
- 中国AIインフラの「CUDA非依存」シフトの核心的議論

> **出典**: 36kr — 梁文鋒インタビュー（2026-07-31）[T1]

### ローカルデプロイコスト論争 — 「没两辆劳斯莱斯幻影，别想部署开源大模型」（7月下旬〜8月）

36krが掲載した記事が中国コミュニティで大論争を引き起こ：

- **主張**: 国産大模型のローカルデプロイには**約3000万元（約6億円）**が必要
- **タイトル**: 「没两辆劳斯莱斯幻影，别想部署开源大模型」（ロールスロイス・ファントム2台持てなければ、开源大模型のデプロイは考えるな）
- **論点**: 64 accelerator以上のスーパーノード構成、保守・運用コスト、人員コストを含む総コスト
- **反論**: 消費者GPU（RTX 5090/4090）での小規模デプロイや量子化済みモデルなら大幅に低コスト可能
- **火山引擎対応**: veStackが**Kimi K3 Day-0適配を完了**。クラウド経由での安価な利用道を開く

> **出典**: 36kr — ローカルデプロイコスト分析（2026-07下旬）[T1]; 火山引擎公式 [T1]

### 国産大模型市場の構造変化 — 3ヶ月で4つのClaudeキラー出現（7月下旬〜8月）

Kimi K3 OSS化を契機に、中国大模型市場の構造的変化が顕著に：

- **コミュニティ爆発**: Kimi K3开源後、GitHub/掘金/V2EXで技術議論が爆発的展開。2.8T MoEの実用的デプロイ方法が数週間で多数提案
- **LLM Space**: 火山引擎がAgent構築ツール**LLM Space**をオープンソース化。中国開発者向けAgent開発エコシステムが加速
- **3ヶ月で4つのClaudeキラー**: 2026年4月〜7月の間に、Claudeに匹敵する性能を持つ国産モデルが4つ登場（Kimi K3、DeepSeek V4、GLM-5、MiniMax M3等）
- **市場転換点**: 国産モデルが「NVIDIA/CUDA依存」から「多様化エコシステム」へ、かつ「API主導」から「ローカル+APIハイブリッド」へ

> **出典**: 掘金・V2EX議論（2026-07下旬〜8月）[T2]; 火山引擎LLM Space公式 [T1]
...
| 5月29日 | NVIDIA VR200 遅延報道 | 冷却問題によりVR200出荷が2ヶ月遅延。中国AIチップ依存加速の可能性 |
| 5月30日 | DeepSeek 制限強化 | 重生・修正回数制限開始。無料版のコスト圧力顕在化。ローカル推論へのシフト動機に |
| 5月31日 | MiniMax M3 発表 | コーディング特化・Agent能力・100万トークンコンテキスト。国産モデルローカル実行の新選択肢 |
| 5月31日 | 上海大模型龙头 A株上場 | MiniMaxがA株上場を申請。中国大模型スタートアップの事業化段階へ |
| 6月1日 | Qwen3.7-Max 無料開放 | API無料アクセス開始。GGUF量子化版ローカルデプロイ未確認 |
| 6月1日 | 百度APP OpenClaw接続 | 百度APPがOpenClawを正式統合、全ユーザー期間限定無料。Agent基盤の大衆化 |

## 2026年5月26日〜6月1日更新 — CANN生態系の成熟と新モデルラッシュ

### CANNエコシステムの転換点 — 「青年期」への移行（5月下旬〜6月）

量子位の報道及び鲲鹏昇腾开发者大会2026の内容から、Huawei CANNエコシステムが大きな転換点を迎えている：

- **CANN全面オープンソース化の成果**: 2025年8月にCANNオープンソース化、同年12月にコンパイラ・ランタイム等コアコード全量公開。65のソースレポが階層分離・疎結合化完了
- **70+大模型がDay-0対応**: DeepSeek V4・GLM-5等の主要モデルがリリース即日で昇腾対応。以前は「3〜4ヶ月の適応待ち」が解消
- **Agentによる開発効率革命**: 従来数ヶ月かかっていた算子適応作業が、AI Agentにより30分以内に短縮。CANN自体の開発にもAgent活用が進む
- **コミュニティ自走の兆し**: 金融機関（股份制银行）がvLLM-Ascendに34の最適化をコントリビュート。創業間もないAIGCodeが昇腾上でMoEモデルMFU 65%達成（業界平均の約2倍）
- **DeepSeek V4が示した「芯模協同」**: DeepSeek V4は大規模・高強度・工学的尺度で昇腾との協調設計の有効性を初めて実証

> **出典**: 量子位 — [鲲鹏昇腾开发者大会2026](https://www.qbitai.com/2026/06/425001.html) [T1]; ChinAI #361 — [CANN分析](https://substack.com/app-link/post?publication_id=2660&post_id=199909074) [T2]

### MiniMax M3 発表 — 国産コーディング特化モデル（5月31日〜6月1日）

MiniMaxが**M3モデル**を正式発表：

- **特徴**: 最先端Coding能力・Agentic能力・100万トークン超長コンテキスト
- **ローカル推論の可能性**: 現時点でGGUF量子化版の有無は未確認。MiniMax M2.7ではUnsloth Dynamic 2.0量子化版が公開されていたため、M3でも同様の展開が期待される
- **Agent連携**: OpenClaw等のAgentフレームワークとの統合が中国開発者コミュニティで話題に
- **事業化**: MiniMaxが上海大模型龙头としてA株上場を申請。中国大模型スタートアップの事業化段階入りを示す

> **出典**: 掘金 — [MiniMax M3深度體驗](https://juejin.cn/post/7646060500482637862) [T2]

### Qwen3.7-Max 無料開放とローカルデプロイの行方（6月1日）

- **ステータス**: Qwen3.7-MaxのAPI無料アクセス開始。性能評価は高いが「使いすぎると高額」との報告
- **ローカルデプロイ**: GGUF/GPTQ量子化版の公開は本日時点で未確認。百煉プラットフォーム経由のAPI提供が優先
- **コミュニティ期待**: 中国開発者の間では「Qwen3.7-Maxのローカル量子化版を待つ」声がV2EX・掘金で散見

> **出典**: 掘金 — [免費的Qwen3.7max終於來了](https://juejin.cn/post/7646234722919170088) [T2]; V2EX議論 [T2]

### DeepSeek サービス制限とローカル推論への影響（5月30日）

- DeepSeek無料版で**重生・修正回数制限**を開始
- 背景: 700億円級の資金調達圧力と推論コスト増大
- **ローカル推論へのシフト**: API制限強化により「Free Rider」層がローカルデプロイに移行する動機が強まる。V2EXではDeepSeek V4 FlashのRTX 5090動作報告や、開悟(Kaiwu)最適化の再評価が活発化
- 企業ユーザーはSGLang+vLLMでの自社ホスティング（8×A100〜昇腾クラスター）を本格検討

> **出典**: 36氪 — [700億融資趕緊到位吧，DeepSeek 開始限制重生](https://36kr.com/p/3831137120395271) [T1]

### NVIDIA VR200 出荷遅延 — 中国AIチップ戦略への波及（5月29日）

- Nvidiaの次世代GPU VR200が**冷却問題で2ヶ月遅延**の報道
- 中国市場ではH20輸出規制とVR200遅延の二重打撃。昇腾950超ノード・摩尔线程MUSA対応へのシフトが加速
- **ローカル推論視点**: 消費者GPU（RTX 5090/4090）の入手性に直接影響は限定的だが、企業のAIインフラ計画に不透明感。今後の中国AIハードウェア戦略の焦点は「CUDA非依存」へ

> **出典**: Tech Taiwan Newsletter — [Nvidia VR200 Reportedly Delayed](https://substack.com/app-link/post?publication_id=3423408&post_id=199714269) [T1]

### 百度APP × OpenClaw 統合（5月31日〜6月1日）
...
### 2026年8月9日〜15日更新 — DeepSeek V4 Pro/Harness OSS・API値上げによるローカル移行加速

#### DeepSeek V4 Pro 正式リリース（8月12-13日）
DeepSeekが**V4 Pro**を正式リリース。1.6Tパラメータ、49B活性化、1Mコンテキスト長のMoEアーキテクチャ。

- **ローカルデプロイ要件**: FP8で865GB VRAM必要（8〜16枚H100クラス）。量子化版Q4なら432GB
- API料金値上げ公告（8/6）とセットで、大規模ユーザーのローカル移行加速
- V2EXで「DeepSeek V4 Pro 正式版已经发布了」のスレッドが立ち、ローカルデプロイ要件が議論

> **出典**: Juejin [T2], V2EX 2026-08-12 [T2]

#### DeepSeek Harness OSS公開（8月13日）
DeepSeek Harness（エージェント実行基盤）のv0.1がMITライセンスでオープンソース化。「一切皆プラグイン」アーキテクチャ。

- エージェントループ自体がプラグイン可能な設計 — ローカルLLMとの統合が容易
- Cordisフレームワーク採用、崔添翼率いるAgentチームが開発
- 2万字の保姆級チュートリアルが掘金で公開

> **出典**: Juejin — [DeepSeek Harness 开发者预览版](https://juejin.cn/post/7673407844636459059)（2026-08-13）[T2]

#### Pi + DeepSeek V4 Flash 実践報告（8月10日）
CLIベースコーディングエージェント「Pi」でV4 Flashを使用した高評価レポート。

- V4 Flash（284B total/13B active）はQ4量子化で80GB、RTX 5090単体で動作可能圏内
- 「这用着也太爽了」との高評価 — コーディングエージェントとしてのローカル利用事例

> **出典**: Juejin — [Pi + DeepSeek-v4-Flash](https://juejin.cn/post/7671964740780785670)（2026-08-10）[T2]

#### KV Cache技術解説（8月10日）
「浅析大模型推理十二篇之KV Cache」— 推論におけるKVキャッシュの技術的深堀り記事。VRAM最適化の基礎技術解説として有用。

> **出典**: Juejin（2026-08-10）[T2]

#### 企業ローカルデプロイ議論（8月14日）
V2EXで「公司本地部署开源模型」スレッド（スコア54）。20〜40人規模の社内デプロイに必要なハードウェア構成が議論。

> **出典**: V2EX — [公司本地部署开源模型](https://www.v2ex.com/t/1234254)（2026-08-14）[T2]

#### AI推論の分散化トレンド（8月8日〜10日）
