...
- **検索バックエンド実測: LSP vs grep（2026-09-19）**: V2EX t/1243146 の個人実測（3モデル×複数 Python/TS リポジトリ、score 1）が「Coding Agent に LSP 语义検索を繋ぐのは grep より必ずしも良くない」を定量報告。要点は4つ: (1) 単純なコード定位ではモデルは LSP をほぼ選ばない（自発選択率 0〜6%）が、強制すると成功率が 100%→89% に低下、(2) 全呼び出し元検索では自発選択率 45〜57% で grep 精确率 0.76→1.00、ただし召回率は双方 0.66 前後で LSP は助けにならない、(3) 同名テキストの多いリポジトリほど LSP の利点が顕在化（hono で grep 精确率 0.51→LSP で F1 +0.246・token 12% 削減、クリーンな remeda では token 16% 増）、(4) **最大の改善は検索後端ではなく返却フォーマット変更** — LSP がファイルパスと行号だけ返していたのをソース本体付きに変えたことで多ファイルリネムの pass@1 が 0.67→0.83、冗長ファイル読み出しが 15.2→3.2 回（純 grep の 4.3 回以下）に減少。本ページの「外部ツールは能力を上げるのでなくタスクの性質を変える」（認知制品理論）の実測側裏付けであり、特に (4) は**ハーネス側の返却設計がモデル選択の良し悪しより効く**ことを示す初期事例。単一ソース・個人実測のため未検証。
- **解説シリーズの細部深化（2026-09-10、参考情報）**: 掘金で同日、「万物皆插件——Cordis 核心设计深度解读」（シリーズ第02章・dsh の Plugin/Context/Service/Event/Effect の5機構）と「Agent 编排 Agent：子代理与工作流系统」（第14章・親エージェントによる子エージェントオーケストレーション）が公開。前者は章番号が第02章へ戻るなど単発記事ではなく継続シリーズであることが確認でき、後者は09-08の沙箱章に続く**マルチエージェント編排**テーマで、シリーズが機能実装→運用安全→オーケストレーションと対象を広げている。いずれも低スコア（0）のプレビューのみで新事実の裏付けはない。併走する「从『跑完即销毁』到『用完再销毁』：云沙箱的一次进化」（score 0）はクラウド沙箱のライフサイクル（実行毎破棄→使い切り後の破棄）を論じるが本文がプレビューのみ・製品名特定できず、参考情報のみ。
- **Claude Code から Pi への移行論（2026-09-22）**: 掘金の大手アカウント JavaGuide（初出 2026-09-20、score 8）が「为什么越来越多人放弃 Claude Code 转而用 Pi？」を公開。Pi は Claude Code 系のハーネスで、さらにその fork で DeepSeek 接続向かとされる OMP も存在するという。スレッドは「Pi の harness は Claude Code と比べて軽量/操作性で利す」という体感中心（本文は冒頭のみ受領、本体未受領）。09-07 以降の Skills 棚卸し論（→ [[agent-skills]]）とは別水位の**ハーネス自体の選択への移動**と読めるが、単一ソース・本文未受領のため未検証（参考情報）。
