...
- `concepts/agent-skills.md` — **更新**: 「続報（2026-09-17 その2）」節追加。OpenAI 公式「Astra 時代 Codex プロンプト掃除」4方針を朝の「負優化」論の公式側再定義として記録（リンク切れ・プレビューのみ）
...
- `concepts/harness-engineering.md` — **更新**: 中国語圏議論動向に2026-09-10条目追加。Cordis 插件框架（第02章）・子代理编排（第14章）・云沙箱ライフサイクル記事を確認、シリーズ対象拡大の傾向として記録（参考情報）
...
- `entities/openai.md` — **更新**: 「続報（2026-09-07その3）」節追加。国企業での代理ソフト禁止によるGPTアクセス困難（V2EX スコア13、未検証）、258k超コンテキスト設定の実用性議論。Day-3反応は吹爆系を集約しアクセス制約のみ記録
- `concepts/coding-plan.md` — **更新**: 「火山方舟 Agent Plan 運用不満噴出（2026-09-07）」節追加。モデル兜底濫用・用量統計不整合・限额・客服対応の4点（単一ソース未検証）。AFP制の実運用信頼性課題として記録
- `entities/gemini-google.md` — **更新**: Gemini 3.8 Flash発表と「又被嘲」反応（6週3世代の迭代頻度、跑分vs実体験の乖離構図の再現、見出しのみ・未検証）
### 本日更新（2026-09-06 Crawl Triage 2: GPT-6 Astra 安全性・課金・ベンチマーク続報）
- `entities/openai.md` — **更新**: 「続報（2026-09-06その2）」節追加。cybersecurity能力Critical閾値到達の公式報道言及・幻觉率9.4%→2.0%と多輪対抗攻撃で防御率67%への下落（いずれも単一ソース・未検証）、単価2.5倍化の実効コスト分析
### 本日更新（2026-09-06 Crawl Triage: GPT-6 Astra続報 / Harnessエコシステム）
- `entities/openai.md` — **更新**: 「続報（2026-09-06）: 混乱への公式対応とBYOK封鎖」節追加。奥特曼の謝罪・全プラン提供完了報道（本文プレビューのみ・未検証）、Pro 20x OSS貢献者無料提供と破甲封鎖事例
- `concepts/harness-engineering.md` — **更新**: 中国語圏議論動向に「DeepSeek Harness周辺エコシステムの成熟」追記。源码解读シリーズ（8+回）・15款プラグイン・Demo→生産実務記事の集中を概念→実装段階移行の兆候として記録
